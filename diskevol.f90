...
program diskevol
  ...
  character(len=100)               :: buffer, label, save_location, arg, params_file
  integer                          :: pos
  integer                          :: ios = 0
  integer                          :: line = 0
...
!   tinfall                      = 1.3d5*year  ! starting from this time, outputs are dumped more often
  ...
  if (len_trim(params_file)/=0) then
    write (*,*) params_file
  else
    write(*,*) 'No file was provided; looking for params.par.'
    params_file = 'params.par'
    ! params_file = TRIM(params_file)
  end if
  !
  ! Read parameters values from the control file
  !
  open(fh,file=TRIM(params_file),action='read')
      print *, 'Digging for ', TRIM(params_file)
  ...
  dusttime = 0.d0
  itsave   = 0
  call diskevol_reset_savefiles(save_location)
  !
  ! First save data of the initial state (itsave=0)
  !
  call save_data(time,itsave,save_location)
  itsave   = 1
  timesave = tsav0
  dt = dtmin
  dtdust = 1.d-4*year
